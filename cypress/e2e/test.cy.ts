...
describe('test cypress-polkadot-wallet plugin', () => {
  ...
  it('should init the wallet and connect accounts with authorization request', () => {
    ...
    // cy.get('#polkadot-js').should('contain', 'polkadot-js');
    // cy.get('#polkadot-js').click();
    // cy.get('#all-accounts').should('contain', shorten(Alice.address));
    // cy.get(`#${ Alice.address }`).click();
    // cy.get('#all-accounts').should('not.exist');
